...
def check_win(player):    # 判断是否有一方胜利（不只可以返回True，未获胜时也得返回False）并画图
    for row in range(0, 3):
        if console_board [row] [0] == player and console_board [row] [1] == player and console_board [row] [2] == player:
            horizontal_winning(row)    # 在当前行画横线
            return True
    for column in range(0, 3):
        if console_board [0] [column] == player and console_board [1] [column] == player and console_board [2] [column] == player:
            vertical_winning(column)    # 在当前列画竖线
            return True
    if console_board [2] [0] == player and console_board [1] [1] == player and console_board [0] [2] == player:
        ascending_diagonal()
        return True
    if console_board [0] [0] == player and console_board [1] [1] == player and console_board [2] [2] == player:
        descending_diagonal()
        return True
    return False
